...
def main():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Quit":
            ...
            # Ensure that boxes are not added
            image_data = transformImg(image_data, add_boxes=False)
            text_result = getText(image_data, False)
            update_image(window, image_data)
            window["-TEXTBOX-"].update(value=text_result)
        elif event == "Show Boxes" and image_data is not None:
            show_boxes = True
            # Draw boxes around words
            img_with_boxes = np.copy(image_data)
            ...
            img_without_boxes = np.copy(image_data)
            ...
            if word_to_remove.strip() != "":
                image_data = findAndRemoveWord(image_data, word_to_remove)
                update_image(window, image_data)
        elif event == "Increase Size" and image_data is not None:
            scale_factor += 1
            image_data = resizeImage(image_data, scale_factor)
            update_image(window, image_data)
